...
const Navbar = () => {
    ...
    return (
        <nav className="sticky top-0 z-50 backdrop-blur-md bg-warm-50/85 border-b border-warm-200/50 transition-all duration-300">
                ...
                <div className="flex items-center justify-between h-20">
                    ...
                    <div className="flex-shrink-0 flex items-center">
                        <Link to="/" className="flex items-center gap-2 group">
                            ...
                            <div className="flex flex-col">
                                <span className="font-serif text-lg md:text-xl font-bold tracking-wide text-secondary-800 group-hover:text-primary-500 transition-colors duration-300">
                                    Kumaon Craft
                                </span>
                                <span className="text-[10px] tracking-[0.2em] uppercase text-primary-600 font-bold -mt-1">
                                    Connect
                                </span>
                            </div>
                        </Link>
                    ...
                    <div className="md:hidden">
                        <button
                            ...
                            type="button"
                            className="inline-flex items-center justify-center p-2 rounded-lg text-secondary-600 hover:text-primary-500 hover:bg-warm-100 transition-all duration-300 focus:outline-none"
            ...
            <div className={`md:hidden overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`} id="mobile-menu">
                <div className="px-4 pt-2 pb-6 space-y-2 bg-warm-100/90 backdrop-blur-md border-t border-warm-200/50">
                    {navLinks.map((link) => (
                        <Link
                            key={link.name}
                            to={link.path}
                            onClick={() => setIsOpen(false)}
                            className={`block px-4 py-3 rounded-xl text-base font-semibold tracking-wide transition-all duration-300 ${isActive(link.path)
                                    ? 'bg-primary-50 text-primary-500 font-bold'
                                    : 'text-secondary-600 hover:bg-warm-200/50 hover:text-primary-500'
                                }`}
                        >
                            {link.name}
                        </Link>
